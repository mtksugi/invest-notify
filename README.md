...
> **本来のゴール（再確認）**: 「次の PLTR / APP / SMCI / VRT / CVNA / RKLB / CLS」級の **テーマ初期〜中盤の中小型株（時価総額 10〜100 億ドル前後）** を、まだ織り込まれていないうちに発掘して通知すること。
>
> 現状の日次パイプラインは「場況把握 + 注視銘柄の動向モニタ」として有用だが、上記ゴールには直接は届いていない。再設計の方向性は [`docs/REDESIGN_v0.3.md`](docs/REDESIGN_v0.3.md) に整理した。次フェーズでは **Multibagger Radar（週次/トリガ駆動）** を主役とし、現在の日次通知を補助系として位置づけ直す予定。
...
- [ ] **Multibagger Radar（v0.3 再設計）**: テーマ初期〜中盤の中小型株を継続的に発掘する週次/トリガ駆動の系統を新設する。詳細は [`docs/REDESIGN_v0.3.md`](docs/REDESIGN_v0.3.md)。
  - [ ] Phase 1: ユニバース生成 / ファンダ取得（米株 / 日本株、時価総額バンド）
  - [ ] Phase 2: スコアリング（売上加速・利益率改善・モメンタム・希薄化チェック）
  - [ ] Phase 3: 芽トリガ通知（連発防止・専用メールテンプレート）
  - [ ] Phase 4: 既存日次パイプラインとの連結（候補銘柄ヒット時に `radar_hit` フラグ）
